      ...
      {activeTab === 'home' ? (
        <>
          ...
          <header className="relative min-h-[760px] md:min-h-[700px] lg:h-[580px] overflow-visible bg-[#EDE6D9]">
            ...
            {/* booking checker */}
            <div className="absolute z-30 left-1/2 -translate-x-1/2 bottom-3 md:-bottom-[2px] w-[92%] max-w-[1215px] bg-[#F8F5EE] shadow-xl border border-[#D8D0C2] rounded-[8px] p-3">
              <div className="grid grid-cols-2 md:grid-cols-[1fr_1fr_1.15fr_1.35fr]">
                <button onClick={()=>setIsBookingOpen(true)} className="px-3 sm:px-5 py-2 text-left md:border-r border-[#D8D0C2]"><span className="block text-[12px] font-serif text-[#777064]">{lang==='de'?'Anreise':lang==='it'?'Arrivo':lang==='nl'?'Aankomst':'Arrival'}</span><span className="font-serif text-[15px]">{selectedCheckIn?selectedCheckIn.toLocaleDateString(lang==='de'?'de-DE':lang==='it'?'it-IT':lang==='nl'?'nl-NL':'en-GB'):(lang==='de'?'Datum wählen':lang==='it'?'Scegli data':lang==='nl'?'Kies datum':'Choose date')}</span></button>
                <button onClick={()=>setIsBookingOpen(true)} className="px-3 sm:px-5 py-2 text-left md:border-r border-[#D8D0C2]"><span className="block text-[12px] font-serif text-[#777064]">{lang==='de'?'Abreise':lang==='it'?'Partenza':lang==='nl'?'Vertrek':'Departure'}</span><span className="font-serif text-[15px]">{selectedCheckOut?selectedCheckOut.toLocaleDateString(lang==='de'?'de-DE':lang==='it'?'it-IT':lang==='nl'?'nl-NL':'en-GB'):(lang==='de'?'Datum wählen':lang==='it'?'Scegli data':lang==='nl'?'Kies datum':'Choose date')}</span></button>
                <button onClick={()=>setIsBookingOpen(true)} className="px-3 sm:px-5 py-2 text-left border-t md:border-t-0 border-[#D8D0C2]"><span className="block text-[12px] font-serif text-[#777064]">{ui.guests}</span><span className="font-serif text-[15px]">{bookingGuest.guests} {ui.guests}</span></button>
                <div className="border-t md:border-t-0 border-[#D8D0C2] pt-2 md:pt-0"><button onClick={()=>setIsBookingOpen(true)} className="w-full h-12 bg-[#66664C] text-white px-6 uppercase tracking-[.14em] text-[11px] flex items-center justify-between">{t.bookStay}<ArrowRight className="w-4 h-4"/></button><div className="text-[9px] text-right text-[#777064] pt-1">{ui.noPayment}</div></div>
              </div>
            </div>
          </header>
